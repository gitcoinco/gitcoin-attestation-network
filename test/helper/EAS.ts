...
} from "./eas-typechain-types/contracts/eip712/proxy/EIP712Proxy";
import {
  AttestationStructOutput,
  MultiDelegatedAttestationRequestStruct,
  MultiDelegatedRevocationRequestStruct,
} from "./eas-typechain-types/contracts/IEAS";
import { NO_EXPIRATION, ZERO_BYTES32 } from "./utils";
import {
  getSchemaUID,
  getUIDFromAttestTx,
...
export const registerSchema = async (
  ...
  const address =
    typeof resolver === "string" ? resolver : await resolver.getAddress();
...
export const expectAttestation = async (
  ...
  switch (signatureType) {
    case SignatureType.Direct: {
      ...
      const returnedUID = await eas
        .connect(txSender)
        .attest.staticCall(...args);
    ...
    case SignatureType.Delegated: {
      ...
      expect(
        await eip712Utils.verifyDelegatedAttestationSignature(
      ...
      const returnedUID = await eas
        .connect(txSender)
        .attestByDelegation.staticCall(...args);
    ...
    case SignatureType.DelegatedProxy: {
      ...
      expect(
        await eip712ProxyUtils.verifyDelegatedProxyAttestationSignature(
          await txSender.getAddress(),
          signature,
        ),
      ).to.be.true;
      ...
      const returnedUID = await eip712ProxyUtils.proxy
        .connect(txSender)
        .attestByDelegation.staticCall(...args);
      res = await eip712ProxyUtils.proxy
        .connect(txSender)
        .attestByDelegation(...args);

      uid = await getUIDFromDelegatedProxyAttestTx(res);
      expect(uid).to.equal(returnedUID);

      expect(await eip712ProxyUtils.proxy.getAttester(uid)).to.equal(
        await txSender.getAddress(),
      );
  ...
  if (!options.skipBalanceCheck) {
    ...
    expect(await getBalance(await txSender.getAddress())).to.equal(
      prevBalance - value - transactionCost,
    );
  }
  ...
  await expect(res)
    ...
    .withArgs(recipient, attester, uid, schema);
...
export const expectMultiAttestations = async (
  ...
  switch (signatureType) {
    case SignatureType.Direct: {
      ...
      const returnedUIDs = await eas
        .connect(txSender)
        .multiAttest.staticCall(...args);
    ...
    case SignatureType.Delegated: {
      ...
      for (const { schema, data } of multiAttestationRequests) {
        const signatures: EIP712Request<
          EIP712MessageTypes,
          EIP712AttestationParams
        >[] = [];
      ...
      const args = [
        multiDelegatedAttestationRequests,
        { value: msgValue },
      ] as const;

      const returnedUIDs = await eas
        .connect(txSender)
        .multiAttestByDelegation.staticCall(...args);
    ...
    case SignatureType.DelegatedProxy: {
      ...
      for (const { schema, data } of multiAttestationRequests) {
        ...
        for (const request of data) {
          const signature =
            await eip712ProxyUtils.signDelegatedProxyAttestation(
              txSender,
              schema,
              request.recipient,
              request.expirationTime,
      ...
      const returnedUIDs = await eip712ProxyUtils.proxy
        .connect(txSender)
        .multiAttestByDelegation.staticCall(...args);
      res = await eip712ProxyUtils.proxy
        .connect(txSender)
        .multiAttestByDelegation(...args);
  ...
  if (!options.skipBalanceCheck) {
    ...
    expect(await getBalance(await txSender.getAddress())).to.equal(
      prevBalance - requestedValue - transactionCost,
    );
  }
  ...
  for (const { schema, data } of multiAttestationRequests) {
    for (const request of data) {
      ...
      await expect(res)
        .to.emit(eas, "Attested")
        .withArgs(request.recipient, attester, uid, schema);
  ...
  switch (signatureType) {
    ...
    case SignatureType.Delegated: {
      ...
      await expect(
        eas
          .connect(txSender)
          .multiAttestByDelegation(multiDelegatedAttestationRequests, {
            value: msgValue,
          }),
      ).to.be.revertedWithCustomError(contract ?? eas, err);
...
export const expectRevocation = async (
  ...
  switch (signatureType) {
    case SignatureType.Direct: {
      res = await eas
        .connect(txSender)
        .revoke({ schema, data: { uid, value } }, { value: msgValue });
    ...
    case SignatureType.Delegated: {
      ...
      expect(
        await eip712Utils.verifyDelegatedRevocationSignature(
          await txSender.getAddress(),
          signature,
        ),
      ).to.be.true;
    ...
    case SignatureType.DelegatedProxy: {
      ...
      expect(
        await eip712ProxyUtils.verifyDelegatedProxyRevocationSignature(
          await txSender.getAddress(),
          signature,
        ),
      ).to.be.true;
  ...
  await expect(res)
    .to.emit(eas, "Revoked")
    .withArgs(attestation.recipient, attester, uid, attestation.schema);
...
export const expectMultiRevocations = async (
  ...
  switch (signatureType) {
    ...
    case SignatureType.Delegated: {
      ...
      res = await eas
        .connect(txSender)
        .multiRevokeByDelegation(multiDelegatedRevocationRequests, {
          value: msgValue,
        });
